...
[![JSR](https://jsr.io/badges/@fragment/i18n)](https://jsr.io/@fragment/i18n) [![JSR Score](https://jsr.io/badges/@fragment/i18n/score)](https://jsr.io/@fragment/i18n)

`@fragment/i18n` provides a dependency-free, strongly typed internationalization runtime and a Cliffy-powered synchronization CLI for frontend, backend, API, and server-rendered TypeScript applications.
...
```bash
deno add jsr:@fragment/i18n
```

Or add it to an npm project through JSR:

```bash
npx jsr add @fragment/i18n
```
...
{
  "tasks": {
    "i18n": "deno --allow-read=. --allow-write=. --allow-net=example.com @fragment/i18n/cli"
  }
}
```
...
```bash
deno install --global --name i18n --allow-read --allow-write --allow-net jsr:@fragment/i18n/cli
```
...
The CLI discovers one shared Deft+ configuration by searching upward from the current directory. It supports `fragment.config.ts`, `.mts`, `.cts`, `.js`, `.mjs`, and `.cjs` in that order. Pass `--config` to use any explicit configuration path.

The default export is a plain object. No `defineConfig` wrapper is required:

```ts
import { i18n } from '@fragment/i18n/config';
...
```bash
deno task i18n sync
deno task i18n sync --config ./configuration/fragment.config.mjs
```
...
```ts
import { createI18n } from '@fragment/i18n';
