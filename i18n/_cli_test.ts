...
Deno.test('findConfigPath() should discover supported files from a child directory', async () => {
  ...
  try {
    ...
    await Deno.writeTextFile(join(directory, 'fragment.config.mjs'), projectConfig());

    expect(await findConfigPath(child)).toBe(join(directory, 'fragment.config.mjs'));
    expect(CONFIG_FILENAMES).toContain('fragment.config.ts');
    expect(CONFIG_FILENAMES).toContain('fragment.config.cts');
    expect(CONFIG_FILENAMES).toContain('fragment.config.js');
    expect(CONFIG_FILENAMES).toContain('fragment.config.cjs');
  } finally {
...
Deno.test('synchronize() should generate resources, declarations, and a runtime factory', async () => {
  ...
  try {
    const configPath = join(directory, 'fragment.config.mjs');
...
`,
    ...
    expect(generatedRuntime).toContain("import projectConfig from '../fragment.config.mjs';");
...
Deno.test('synchronize() should generate a resource without custom types', async () => {
  ...
  try {
    await Deno.writeTextFile(
      join(directory, 'fragment.config.mjs'),
...
Deno.test('createI18n() should run from generated resources and shared TypeScript config', async () => {
  ...
  try {
    await Deno.writeTextFile(
      join(directory, 'fragment.config.ts'),
...
Deno.test('findConfigPath() should propagate unexpected filesystem errors', async () => {
  ...
  await expect(findConfigPath(Deno.cwd(), './fragment.config.ts')).rejects.toBe(failure);
...
Deno.test('findConfigPath() should prioritize TypeScript configuration', () => {
  expect(basename(CONFIG_FILENAMES[0])).toBe('fragment.config.ts');
